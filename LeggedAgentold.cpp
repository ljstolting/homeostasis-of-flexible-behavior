...
void LeggedAgent::StepCPG(double StepSize, bool adaptpars)
{
	...
	if (NervousSystem.NeuronOutput(1) > 0.5) {Leg.FootState = 1; Leg.Omega = 0;} //curious why omega reset had been removed here but not in others
...
//			force = Leg.ForwardForce - Leg.BackwardForce;
	...
	// If the foot is too far back, the body becomes "unstable" and forward motion ceases. vice versa for too far forward
	if ((cx - Leg.FootX > 20) || (Leg.FootX - cx > 20)) vx = 0.0;
}


// Step the insect using a general CTRNN RPG (note sensor weights not evolved)
...
// 	// Update the nervous system
// 	NervousSystem.EulerStep(StepSize,0);
// 	// Update the leg effectors
...
// AMMENDED TO BE FT AND FORCE INSTEAD OF FS AND BS
...
void LeggedAgent::Step2CPG(double StepSize, bool adaptpars)
{
	...
	// I believe this is equivalent to previous and unneccessary because of absolute value...
...
// void LeggedAgent::Step1RPG(double StepSize)
// {
// 	double force = 0.0;
